...
// url that gives a JSON object of all beverages
		...
		jQuery.get(url, function (data){
			...
			// In the newly created div there is an imageDiv placed which contains a static picture for now (heineken.jpg). 
			//The ID of the picture is a name from the beer that is used in the current  loop (e.g. data.payload[16].namn)
...
// below is an Flickr function that searches for pictures using the beername as a keyword. 
//The results aren't really satisfying so we will manually download pictures for the beers :)
  /*
...
*/
       ...
       //alert();
			...
			//$("#b_title").style.visibility="hidden";
			...
			// when clicking on a picure of a beer in the class 'picture', the variable beerID is made. 
			//The ID of the button clicked is attached to that variable.
			...
			$('.picture').click(function() {
				...
				if ($("li[id*='" + beerID + "']").length) {
					...
					//$("#amount ul").append("<input type="Number"  value="1"/>");
					totalPrice = count * beerPrice;
